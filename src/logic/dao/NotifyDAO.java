...
public class NotifyDAO {
	...
	public static synchronized void addNotify(ObservableNotify notify) {
		try {
			con = ConnectionDB.dbConn();
			try(Statement tempSt = con.createStatement()){
				String sql = "UPDATE notifica SET valoreConosciuto = '" + notify.getEntityValoreConosciuto() +
						"' WHERE username = '" + notify.getEntityUsername()+ "';";
	...
	public static synchronized int getValoreNotifica(Message message) {
		try {
			con = ConnectionDB.dbConn();
			try(Statement tempSt = con.createStatement()){
			...
			try(Statement tempSt = con.createStatement()){
				...
				if (tempRs.first()) {
					...
					if(newValue > oldValue) {
							...
							notify.setEntityNotice(difference,username);
					}
				}
			}				
			con.close();
