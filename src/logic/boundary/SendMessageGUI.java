...
public class SendMessageGUI extends WindowManager {
		...
		if (messages.size() >= 1) {
			...
			for (int i = 0; i < messages.size(); i++) {
				...
				messageArea.setText(messageArea.getText() + Integer.toString(i + 1) + "\nMittente: "
						+ messages.get(messages.size() - 1 - i).getMittente() + "\nRicevente: "
						+ messages.get(messages.size() - 1 - i).getRicevente() + "\nTesto: "
						+ messages.get(messages.size() - 1 - i).getTesto() + "\n\n");
			}
		} else {
	...
	public void send() {
		...
		try {
			...
			res = controllerSendMessage.insertText(messageBean);
			if (res == -1) {
	...
	@FXML
	public void homePageSMClick() {
		homePageSM();
	}

	@FXML
	public void logoutSMClick() {
		logoutSM();
	}

	@FXML
	public void profileSMClick() {
		profileSM();
	}
