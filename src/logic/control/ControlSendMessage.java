...
public class ControlSendMessage {
	...
	public List<String> showUsers(String loggedUser) {
		...
		while(!userlist.get(i).equals(loggedUser)) {
	...
	public List<MessageBean> showMessages(MessageBean messageBean) {
		...
		for (int i = 0; i < messages.size(); i++) {
			...
			messageBean2.setMittente(messages.get(i).getMittente());
			messageBean2.setRicevente(messages.get(i).getRicevente());
			messageBean2.setTesto(messages.get(i).getTesto());

			messageBeanList.add(messageBean2);
		}
